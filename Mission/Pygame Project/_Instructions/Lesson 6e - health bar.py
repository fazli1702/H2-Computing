...
###### LESSON 6e ######
#In this lesson, we will define a health variable for the target
#and display the health bar on the game screen.
#When the target is hit by the bullet, the health value will decrease;
#the target will be killed and disappear when the health value falls below zero.
#
...
def main():
    ...
    birdsound=pygame.mixer.Sound("birdsound.wav")  #load boom.wav sound file
    birdsound.set_volume(100)  
    ...
    targetx = 300 #initialise target's x coord
    targety = 150 #initialise target's y coord

    health = 50  #initialise the target's health value

    #image size : (34,24)
    bird = pygame.image.load('bird.png') #load the image as a target
...
##                    print(bullets)
    ...
    def process_game_objects():
        nonlocal xloc, yloc, GAMESTATE, bullets, health #declare game variables you are going to access
        ...
        for i in range (len(bullets)):  #for every bullet in the list
            ...
            if health > 0:     
                if targetx<bullets[i][0]<targetx+34 and targety<bullets[i][1]<targety+24:
                    birdsound.play() #play the birdsound when target is hit
                    print("Ouch") #we hit the target!
                    bullets[i][0]+=400 #bullet will 'disappear' after hitting the target

                    health -= 10 #deduct health on hit
                    if health <= 0:
                        print("Target killed!")
    ...
    ############################
    def display():
        nonlocal xloc, yloc, GAMESTATE, health #declare game variables you are going to access
        ...
        # Draw the target bird
        if health > 0:
            screen.blit(bird, (targetx,targety))  #draw the target

        # Draw health bar
        screen.blit(font.render("Health", True, (255,0,0)), (150,5))
        pygame.draw.rect(screen,(255,0,0),pygame.Rect(220,10,health,10)) 
